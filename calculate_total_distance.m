function total_distance = calculate_total_distance(od_matrix, individual)
...
%
%    输入参数
%        od_matrix - n x n 的 OD 矩阵，表示各个节点间的距离
...
for i = 1:length(individual)
    ...
    % 由于在 OD 矩阵中第一行第一列代表的是 CVRP 问题的仓库 Depot 的坐标，因此需
    % 对行列索引各自加上 1 跳过第一行第一列
    total_distance = total_distance + od_matrix( ...
...
% 最后从最后一个节点回到节点 0
total_distance = total_distance + od_matrix(current_node + 1, 1);
