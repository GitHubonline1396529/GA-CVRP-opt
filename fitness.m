function fitnessValues = fitness(pop, OD_mat, numCustomers, numVehicles, demands, capacity)
% FITNESS 遗传算法解决 VRP 问题的适应度函数计算
%    以列向量的形式返回每一个个体的适应度函数值。在本方法中，使用距离的倒数作为适
%    应度值，从而实现在 VRP 问题中最小化函数目标。
% 
%    fitnessValues = fitness(pop, OD_mat, numCustomers, numVehicles, demands, capacity
% )
%    
%    输入参数
%        pop - 当前的种群矩阵
%        OD_mat - VRP 问题中所有节点的距离矩阵
...
for i = 1 : popSize
    total_dist = calculate_total_distance(OD_mat, pop(i, :));
